...
/**
 * AI planning is deliberately separate from graph mutation. The model may
 * interpret the user's intent, but it never gets to invent operations or
 * credentials. The caller grounds the plan against the real catalog before
 * applying anything to the workflow graph.
 */
export async function planCopilotIntent(opts: {
  ...
  const result = await completeAi({
    ...
    system: [
      ...
      "The trigger must describe the event that starts the workflow.",
      "Each step must describe one meaningful action or control-flow intent in execution order.",
      "Use intentKind=ai for summarization, classification, extraction, generation, transformation or other AI work; use agent only when the user explicitly asks an autonomous agent to act.",
      "Do not invent app operation identifiers. The catalog is grounding context; appHint should be null when uncertain.",
      ...
      "Do not output chain-of-thought or hidden reasoning. summary must be a short user-safe explanation of the intended workflow."
    ].join(" ")
  });

  ...
  return {
    flowRuns: {
      ...
      async checkpoint(runId: string, input: { expectedCursor: number; expectedEpoch: number; appendContext: Record<string, unknown>; nextCursor: number; status: string }) {
        const result = await db.service.query(
          `UPDATE flow_runs SET context = context || $3::jsonb, cursor = $4, status = $5, transition_epoch = transition_epoch + 1 WHERE id = $1 AND cursor = $2 AND transition_epoch = $6 RETURNING *`,
      ...
      async pause(runId: string, input: { expectedCursor: number; expectedEpoch: number; contextJson: Record<string, unknown>; reason: string; resumeAt: string | null; nextCursor?: number }) {
        ...
        const result = await db.service.query(
          `UPDATE flow_runs SET status = 'paused', paused_reason = $3, resume_at = $4, context = $5::jsonb, cursor = $6 WHERE id = $1 AND cursor = $2 AND transition_epoch = $7`,
      ...
      async resumeClaim(runId: string) {
        const result = await db.service.query(
          `UPDATE flow_runs SET status = 'running', paused_reason = NULL, transition_epoch = transition_epoch + 1 WHERE id = $1 AND status = 'paused' RETURNING id, created_at, org_id, project_id, flow_id, flow_version_id, trigger_kind, status, context, transition_epoch, cursor`,
          [runId]
        );
        const row = result.rows[0];
        return row ? mapRun(row) : null;

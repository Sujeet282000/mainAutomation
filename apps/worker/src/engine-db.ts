      ...
      async currentPublished(flowId: string) {
        const result = await db.service.query(
          `SELECT v.* FROM flow_versions v
           JOIN flows f ON f.published_version_id = v.id
           WHERE f.id = $1
           LIMIT 1`,
          [flowId],
        );
...
function mapRun(row: any) {
  ...
  return {
    ...
    status: row.status,
    // The engine contract calls this contextJson. Keep context as an alias for
    // compatibility with callers that inspect the mapped run directly.
    contextJson: context,
    context,

...
import { ArrowRight, Calendar, CheckCircle2, FileSpreadsheet, Mail, MessageSquare, Plus, Sparkles, Table2, Webhook, WandSparkles, Zap } from "lucide-react";
...
export default function AiPage() {
  ...
  return <div className="mx-auto max-w-5xl pb-10">
    ...
    <section className="relative mt-4 overflow-hidden rounded-3xl border border-violet-200/70 bg-gradient-to-br from-violet-50 via-white to-blue-50 p-6 shadow-sm sm:p-8">
      <div className="pointer-events-none absolute -right-16 -top-16 h-48 w-48 rounded-full bg-violet-200/30 blur-3xl" />
      <div className="relative max-w-3xl">
        <div className="mb-4 inline-flex items-center gap-2 rounded-full border border-violet-200 bg-white/80 px-3 py-1 text-xs font-semibold text-violet-700"><Sparkles className="h-3.5 w-3.5" /> AI workflow builder <span className="h-1.5 w-1.5 rounded-full bg-emerald-500" /> Ready</div>
        <h1 className="text-3xl font-semibold tracking-tight text-ink sm:text-4xl">What do you want to automate?</h1>
        <p className="mt-2 max-w-2xl text-sm leading-relaxed text-ink-muted">Describe the outcome you want. Copilot will turn your idea into a real workflow with triggers, actions, and mappings.</p>
      </div>
      <form className="relative mt-7 rounded-2xl border border-violet-200 bg-elevated p-3 shadow-xl shadow-violet-500/5" onSubmit={(e) => { e.preventDefault(); void build(); }}>
      ...
      {building && <div className="relative mt-3 flex items-center gap-3 rounded-xl border border-violet-200 bg-violet-50/70 px-4 py-3 text-xs text-violet-800"><span className="relative flex h-5 w-5 items-center justify-center"><span className="absolute h-5 w-5 animate-ping rounded-full bg-violet-400/30" /><Sparkles className="relative h-3.5 w-3.5" /></span><div><p className="font-semibold">Copilot is building your draft</p><p className="text-violet-700/70">Mapping apps, events, and fields…</p></div></div>}
    ...
    <section className="mt-8 rounded-3xl border border-line bg-elevated p-5 sm:p-6">
      <div className="flex items-end justify-between gap-3"><div><div className="flex items-center gap-2"><span className="flex h-8 w-8 items-center justify-center rounded-xl bg-violet-600/10 text-violet-600"><Sparkles className="h-4 w-4" /></span><h2 className="text-base font-semibold">Need a starting point?</h2></div><p className="mt-1 text-xs text-ink-muted">Pick a popular automation, then customize the idea in your own words.</p></div><span className="rounded-full bg-muted px-2.5 py-1 text-[10px] font-medium text-ink-muted">6 popular ideas</span></div>
      <div className="mt-5 grid gap-3 md:grid-cols-2">
        {examples.map(({ label, description, icon: Icon, prompt: examplePrompt }) => <button key={label} type="button" disabled={building} onClick={() => { setPrompt(examplePrompt); void build(examplePrompt); }} className="group relative overflow-hidden rounded-2xl border border-line bg-muted/20 p-4 text-left transition-all duration-200 hover:-translate-y-1 hover:border-violet-300 hover:bg-violet-50/40 hover:shadow-lg hover:shadow-violet-500/10 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-violet-400 disabled:pointer-events-none disabled:opacity-60"><div className="flex items-start gap-3"><span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-elevated text-violet-600 shadow-sm transition-all group-hover:scale-105 group-hover:bg-violet-600 group-hover:text-white"><Icon className="h-4 w-4" /></span><span className="min-w-0 flex-1"><span className="block text-sm font-semibold text-ink group-hover:text-violet-900">{label}</span><span className="mt-1 block text-xs leading-relaxed text-ink-muted">{description}</span></span><ArrowRight className="mt-1 h-4 w-4 shrink-0 text-ink-muted transition-transform group-hover:translate-x-1 group-hover:text-violet-600" /></div></button>)}
      </div>
    </section>
    <section className="mt-6 grid gap-3 sm:grid-cols-3">
      {[{ icon: Sparkles, title: "Describe", body: "Start with the result you want. You don't need to know every technical field." }, { icon: CheckCircle2, title: "Review", body: "Check apps, triggers, actions, fields, and connections before building." }, { icon: WandSparkles, title: "Test", body: "Run a sample and verify the workflow before you put it live." }].map(({ icon: Icon, title, body }) => <div key={title} className="rounded-2xl border border-line bg-elevated p-4"><Icon className="h-4 w-4 text-violet-600" /><p className="mt-3 text-sm font-semibold">{title}</p><p className="mt-1 text-xs leading-relaxed text-ink-muted">{body}</p></div>)}
    </section>
  </div>;
}

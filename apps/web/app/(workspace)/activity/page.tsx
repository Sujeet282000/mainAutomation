...
export default function ActivityPage() {
  ...
  const list = useQuery({ queryKey: ["executions"], queryFn: () => api<{ executions: Run[] }>("/executions") });
  const items = useMemo(() => {
  ...
  return (
    <div>
      <PageHeader
        title="Activity"
        description="Zap History-style run log: every trigger, status, duration, and error in this workspace."
      />
      ...
      <div className="mb-4 flex flex-wrap gap-2">
        ...
        <Button variant="secondary" size="sm" onClick={() => list.refetch()} disabled={list.isFetching}><RefreshCw className={cn("h-3.5 w-3.5", list.isFetching && "animate-spin")} /> Refresh</Button>
      </div>
      ...
      {!list.isLoading && !items.length && (
        <EmptyState
          ...
          title="No runs yet"
          description="Publish a workflow and fire its trigger, or use Test workflow in the editor. Demo templates include Calendar and OpenAI tests."
        />
      )}
      {view === "cards" ? (
        <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-3">
          {items.map((r) => (
            <Link key={r.id} href={`/activity/${r.id}`} className="rounded-2xl border border-line bg-elevated p-4 shadow-sm transition hover:-translate-y-0.5 hover:shadow-card">
              <div className="flex items-start justify-between gap-3"><div className="min-w-0"><p className="truncate font-medium">{r.automation_name ?? "Run"}</p><p className="mt-1 text-xs text-ink-muted">{r.trigger_type ?? "manual"} · {new Date(r.created_at).toLocaleString()}</p></div><StatusBadge status={r.status} /></div>
              <div className="mt-4 flex items-center justify-between border-t border-line pt-3 text-xs text-ink-muted"><span>{duration(r)}</span><span className="font-medium text-violet-700">Open run →</span></div>
            </Link>
          ))}
        </div>

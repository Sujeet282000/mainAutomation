...
type AuthField = { key: string; label: string; type: string; required?: boolean; placeholder?: string; help?: string; helpUrl?: string; helpUrlLabel?: string; options?: { label: string; value: string }[] };
type AuthSchema = { authType: string; fields: AuthField[]; oauthProvider?: string; appName?: string; capabilities?: { triggers: number; actions: number; searches: number } };
type Conn = { id: string; name: string; status: string; appSlug?: string; app_slug?: string; zap_count?: string };

export function ConnectAccountModal({ appSlug, appName, returnTo, replaceConnectionId, onClose, onConnected }: { appSlug: string; appName?: string; returnTo?: string; replaceConnectionId?: string | null; onClose: () => void; onConnected: (connectionId: string) => void }) {
  ...
  const create = useMutation({
    mutationFn: async () => {
      ...
      const saved = replaceConnectionId
        ? await api<{ connection: { id: string } }>(`/connections/${replaceConnectionId}`, { method: "PATCH", body: JSON.stringify({ credentials }) })
        : await api<{ connection: { id: string } }>("/connections", { method: "POST", body: JSON.stringify({ appSlug, name, authType: schema?.authType, credentials }) });
      const id = saved.connection?.id ?? replaceConnectionId;
      if (!id) throw new Error("Connection was not saved.");
      try { await api(`/connections/${id}/test`, { method: "POST" }); }
      catch (err) { throw new Error(`${err instanceof Error ? err.message : "Connection test failed"}. The account was saved — reconnect with valid credentials, then test the step.`); }
      return { connection: { id } };
    },
    onSuccess: (data) => { qc.invalidateQueries({ queryKey: ["connections"] }); if (data.connection?.id) onConnected(data.connection.id); },
    onError: (err: Error) => setError(err.message)
  });

  async function startGoogle() {
    const token = getToken(); const ws = getWorkspaceId(); const returnQuery = returnTo ? `&returnTo=${encodeURIComponent(returnTo)}` : "";
    const res = await fetch(`${API_URL}/oauth/google/start?appSlug=${encodeURIComponent(appSlug)}${returnQuery}`, { headers: { authorization: `Bearer ${token}`, "x-workspace-id": ws ?? "" } });
    const d = await res.json();
    if (!d.url) { setError(d.error ?? "Google OAuth is not configured."); return; }
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-ink/40 p-4" onClick={onClose}>
      <div className="w-full max-w-md overflow-hidden rounded-2xl border border-line bg-elevated shadow-card" onClick={(e) => e.stopPropagation()}>
        <div className="flex items-start justify-between px-5 pt-5"><div><p className="text-xs text-ink-muted">Connect to Orchestra</p><h2 className="mt-1 text-lg font-semibold">{appName ?? schema?.appName ?? appSlug}</h2>{schema?.capabilities && <p className="mt-1 text-[11px] text-ink-muted">{schema.capabilities.triggers} triggers · {schema.capabilities.actions} actions · {schema.capabilities.searches} searches</p>}</div><button type="button" className="text-ink-muted" onClick={onClose} aria-label="Close">×</button></div>

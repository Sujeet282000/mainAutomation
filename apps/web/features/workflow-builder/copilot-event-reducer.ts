...
type OperationStatus = "pending" | "running" | "completed" | "failed" | "waiting_for_user";
...
function operationStatus(value: unknown): OperationStatus {
  ...
  return status === "pending" || status === "running" || status === "completed" || status === "failed" || status === "waiting_for_user"
    ? status
    : "pending";
}

/** Convert streamed backend events into durable, renderable UI state. */
export function reduceCopilotEvent(
  ...
  if (type === "agent_started") return { ...state, status: "understanding", title: "Understanding your request" };
  ...
  if (type === "reasoning") {
    ...
    return upsertActivity(state, { id: `progress-${state.activities.length + 1}`, kind: "info", label: text, timestamp: Date.now() });
  }

  if (type === "analysis_summary") {
    return appendBlock(state, { type: "analysis_summary", title: String(ev.title ?? "Understanding your request"), items: Array.isArray(ev.items) ? ev.items.map(String) : [] });
  }
  ...
  if (type === "operation_started" || type === "operation_completed") {
    ...
    return { ...state, operationIndex: { ...state.operationIndex, [operationId]: state.blocks.length }, blocks: [...state.blocks, block] };
  }
  ...
  if (type === "connection_required") {
    return appendBlock(state, {
      ...
      actions: actionList(ev.actions) ?? [{ type: "connect_account", label: `Connect ${String(ev.appName ?? ev.appSlug)}`, appSlug: String(ev.appSlug), stepId: typeof ev.stepId === "string" ? ev.stepId : undefined }],
    });
  }
  ...
  if (type === "blocking_issue") {
    return { ...state, status: "blocked", title: "Needs your attention", blocks: [...state.blocks, { type: "warning", title: String(ev.title ?? "Needs your attention"), message: typeof ev.detail === "string" ? ev.detail : undefined, actions: actionList(ev.actions) }] };
  }

  if (type === "agent_error") {
    return { ...state, status: "error", title: "Something went wrong", blocks: [...state.blocks, { type: "error", title: "Something went wrong", message: typeof ev.message === "string" ? ev.message : undefined, actions: actionList(ev.actions) }] };
  }

  if (type === "agent_completed") {
    return { ...state, status: "completed", title: "Done", blocks: [...state.blocks, { type: "success", title: "Completed", message: typeof ev.summary === "string" ? ev.summary : undefined, actions: actionList(ev.actions) }] };
  }

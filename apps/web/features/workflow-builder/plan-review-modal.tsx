...
import { AlertTriangle, ArrowRight, Bot, Check, ChevronDown, ChevronUp, Database, ExternalLink, FileText, Grid3X3, Layers, Loader2, MessageSquare, Plug, Sparkles, X, Zap } from "lucide-react";
...
/** A friendly app name from a missing-connection string like "Connect gmail — Copilot cannot create credentials." */
function appNameFromConn(conn: string): string {
  const m = conn.match(/connect\s+([a-z0-9\-_ ]+?)(?:\s*[—–-]|\s*$)/i);
  return (m?.[1] ?? conn).trim();
}

/** Skip internal/engineering fallback notes that must not reach users. */
function isInternalNote(text: string): boolean {
  return /ai plane|catalog engine|terminated|heuristic planner|fallback/i.test(text);
}
...
export function PlanReviewModal({ open, plan, loading, error, onConfirm, onCancel, onEdit, onClarify }: { open: boolean; plan: CopilotPlanResult | null; loading: boolean; error: string | null; onConfirm: () => void; onCancel: () => void; onEdit: () => void; onClarify?: (answers: Record<string, string>) => void }) {
  ...
  const rawSummary = enhancedPlan?.summary ?? plan?.preview?.summary ?? plan?.reply ?? "";
  const summary = isInternalNote(rawSummary) ? (plan?.preview?.steps?.length ? `I'll build a workflow: ${plan!.preview!.steps.map((s: any) => s.label).join(" → ")}` : "I've planned your workflow. Review the steps below.") : rawSummary;
  ...
  const realQuestions = questions.filter((q) => !/^connect\b/i.test(q.question));
  const hasQuestions = realQuestions.length > 0;
  const allRequiredAnswered = realQuestions.filter((q) => q.required).every((q) => clarificationAnswers[q.question]?.trim());
  return <div className="fixed inset-0 z-[60] flex items-center justify-center bg-ink/60 p-4 backdrop-blur-[2px]">
    <div className="w-full max-w-2xl overflow-hidden rounded-3xl border border-line bg-elevated shadow-2xl" onClick={(e) => e.stopPropagation()}>
      ...
      {plan && !loading && <div className="max-h-[65vh] overflow-y-auto px-6 py-5">
        <div className="rounded-2xl border border-violet-200/70 bg-gradient-to-br from-violet-50/80 to-transparent p-4"><div className="flex gap-3"><span className="mt-0.5 flex h-7 w-7 shrink-0 items-center justify-center rounded-lg bg-violet-600/10 text-violet-600"><Bot className="h-4 w-4" /></span><div className="flex-1"><div className="flex items-center gap-2"><p className="text-sm font-semibold text-violet-900">Here&apos;s the plan</p><AssetTypeBadge type={assetType} /></div><div className="mt-1.5 text-sm leading-relaxed text-ink"><FormattedCopilotMessage text={summary} /></div></div></div></div>
        {realQuestions.length > 0 && <div className="mt-4 rounded-2xl border border-amber-200 bg-amber-50/60 p-4"><p className="text-sm font-semibold text-amber-900">A few details before building</p><div className="mt-3 space-y-3">{realQuestions.map((q, qi) => <div key={qi}><p className="text-xs font-medium text-ink">{q.question}</p>{q.options?.length ? <div className="mt-2 flex flex-wrap gap-1.5">{q.options.map((opt) => <button key={opt} type="button" className={cn("rounded-full border px-2.5 py-1 text-[11px] transition", clarificationAnswers[q.question] === opt ? "border-amber-500 bg-amber-100 font-medium text-amber-800" : "border-line bg-elevated text-ink hover:border-amber-300")} onClick={() => setClarificationAnswers((p) => ({ ...p, [q.question]: opt }))}>{clarificationAnswers[q.question] === opt && <Check className="mr-1 inline h-2.5 w-2.5" />}{opt}</button>)}</div> : <input type="text" className="mt-1.5 w-full rounded-xl border border-line bg-elevated px-3 py-2 text-xs outline-none focus:border-amber-400" placeholder="Type your answer…" value={clarificationAnswers[q.question] ?? ""} onChange={(e) => setClarificationAnswers((p) => ({ ...p, [q.question]: e.target.value }))} />}</div>)}</div></div>}
        {missingConns.length > 0 && <div className="mt-4 rounded-2xl border border-amber-200 bg-amber-50/50 p-4"><p className="text-xs font-semibold text-amber-900"><Plug className="mr-1 inline h-3.5 w-3.5" />Connect these apps so your workflow can run</p><div className="mt-3 space-y-2">{missingConns.map((item: string, i: number) => { const appName = appNameFromConn(item); return <div key={i} className="flex items-center gap-3 rounded-xl border border-amber-100 bg-elevated px-3 py-2.5"><span className="flex h-7 w-7 shrink-0 items-center justify-center rounded-lg bg-amber-100 text-amber-700"><Plug className="h-3.5 w-3.5" /></span><div className="min-w-0 flex-1"><p className="truncate text-xs font-medium capitalize text-ink">{appName}</p><p className="text-[10px] text-ink-muted">Connect once — reused in every workflow</p></div><a href="/connections" className="inline-flex shrink-0 items-center gap-1 rounded-lg bg-amber-600 px-2.5 py-1.5 text-[11px] font-semibold text-white transition hover:bg-amber-700">Connect <ExternalLink className="h-3 w-3" /></a></div>; })}</div><p className="mt-2.5 text-[10px] text-amber-700/80">You can also connect later — steps will show what they need before publishing.</p></div>}
            ...
            <div className="mt-2 space-y-1">
              {enhancedPlan.connections.map((conn: any, ci: number) => (
                <div key={ci} className="flex items-center gap-2 text-[11px]">
                  ...
                  <span className="text-ink-muted capitalize">{conn.appSlug}</span>
                  <span className={conn.status === 'connected' ? 'text-ok' : 'text-warn'}>{conn.status === 'connected' ? 'Connected' : 'Needs setup'}</span>
                </div>
              ))}
            </div>
          </div>
        ...
        {missingInfo.length > 0 && <div className="mt-3 rounded-2xl border border-line bg-muted/40 p-4"><p className="text-xs font-semibold">Good to know</p><ul className="mt-2 space-y-1">{missingInfo.map((item: string) => <li key={item} className="text-xs text-ink-muted">• {item}</li>)}</ul></div>}
      ...
      <div className="flex items-center justify-between border-t border-line bg-muted/20 px-6 py-4"><p className="text-[10px] text-ink-muted">Review first · Copilot won&apos;t publish or sign in for you</p><div className="flex gap-2"><Button size="sm" variant="secondary" onClick={onEdit}>Edit request</Button>{hasQuestions && onClarify && <Button size="sm" variant="secondary" onClick={() => onClarify(clarificationAnswers)} disabled={loading}>Update plan</Button>}<Button size="sm" onClick={onConfirm} disabled={loading || !plan || !allRequiredAnswered} className="bg-violet-600 text-white hover:bg-violet-700">{loading ? <Loader2 className="mr-1 h-3 w-3 animate-spin" /> : <Sparkles className="mr-1 h-3 w-3" />}{hasQuestions && !allRequiredAnswered ? "Answer details" : "Approve & build"}</Button></div></div>
    </div>
  </div>;
}

 ...
 */
export const INVARIANT_1_RUN_PINNING = "A run pins exactly one immutable flow version";
 ...
 */
export const INVARIANT_2_CREDENTIAL_ISOLATION =
  "Credentials never enter a flow definition, queue job, log, model prompt, or Python service";
 ...
 */
export const INVARIANT_3_APPEND_ONLY_CONTEXT = "Execution context is append-only";
 ...
 */
export const INVARIANT_4_IDEMPOTENT_SIDE_EFFECTS =
  "At-least-once delivery with idempotent side effects";
 ...
 */
export const INVARIANT_5_AI_ADVISORY =
  "AI is advisory at build time and bounded at run time";
 ...
 */
export const INVARIANT_6_TOOL_EXECUTION_IN_NODE =
  "Tool execution always happens in Node, never in Python";
 ...
 */
export const INVARIANT_7_DUAL_TENANT_ISOLATION =
  "Tenant isolation enforced twice: application code + Postgres RLS";
 ...
 */
export const INVARIANT_8_IMMUTABLE_VERSIONS =
  "Versions and audit logs are immutable at the database level, by trigger";
 ...
 */
export const INVARIANT_9_AI_ATTRIBUTION =
  "Every AI call is attributed to org, flow, purpose, and run/session";
 ...
 */
export function assertCapability(capability: string): void {
  if (
    (COPilot_CAPABILITIES.mayNot as readonly string[]).includes(capability)
  ) {
    throw new Error(`COPILOT_FORBIDDEN: ${capability}`);
  }
  if (
    !(COPilot_CAPABILITIES.may as readonly string[]).includes(capability)
  ) {

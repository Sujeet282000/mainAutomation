import type { Condition } from "./flow-schema";
...
export function readPath(ctx: Ctx, path: string): unknown {
  ...
  if (ctx.steps && typeof ctx.steps === "object") {
    ...
    if (stepKey) return keys.length === 1 ? steps[stepKey] : walk(steps[stepKey], keys.slice(1));
  }
  return undefined;
}
...
export function resolveValue(value: unknown, ctx: Ctx): unknown {
  if (typeof value === "string") {
    ...
    if (only) return evalToken(ctx, only[1]);
    return value.replace(TOKEN, (_m, e: string) => {
  ...
  if (value && typeof value === "object") {
    return Object.fromEntries(Object.entries(value as Record<string, unknown>).map(([k, v]) => [k, resolveValue(v, ctx)]));
  }
  return value;
}
...
export function evaluateFlowCondition(c: Condition, ctx: Ctx): boolean {
  switch (c.op) {
    case "and":
  ...
  switch (c.op) {
    ...
    default:
      throw new Error(`UNKNOWN_OP:${(c as { op: string }).op}`);
  }
}

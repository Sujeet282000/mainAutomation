...
function backoff(attempt: number, policy: { backoff: "fixed" | "exponential"; initialDelayMs: number; maxDelayMs: number }): number {
  const base = policy.backoff === "fixed" ? policy.initialDelayMs : Math.min(policy.initialDelayMs * Math.pow(2, attempt - 1), policy.maxDelayMs);
...
export class Executor {
  ...
  private async executeDelay(run: EngineRun, cursor: number, context: Record<string, unknown>, step: { id: string; props?: { mode?: string; seconds?: number; untilIso?: string } }): Promise<void> {
    ...
    if (props.mode === "until" && props.untilIso) {
      const at = new Date(props.untilIso); resumeAt = Number.isNaN(at.getTime()) ? null : at.toISOString();
    } else {
      const seconds = Math.max(1, Math.floor(Number(props.seconds ?? 0))); if (seconds > 0) resumeAt = new Date(Date.now() + seconds * 1_000).toISOString();
    }
  ...
  private async executeSubFlow(run: EngineRun, definition: TFlowDefinition, step: { id: string; props?: { flowId?: string; input?: Record<string, unknown>; waitForCompletion?: boolean } }, cursor: number, context: Record<string, unknown>): Promise<void> {
    ...
    if (!childVersion || !childVersion.definition) return this.finish(run.id, "failed", { ...context, [step.id]: { error: { code: "SUBFLOW_NOT_PUBLISHED", message: `Flow ${childFlowId} has no published version` } } });
    ...
    for (const childStep of childDef.steps) {
      try { const outcome = await this.executeStepRecursive(run, childDef, childStep, childContext, 1); if (outcome === "halt") break; }
      catch (err) { childStatus = "failed"; childError = err instanceof EngineError ? err : EngineError.from(err); break; }
    }
  ...
  private async executeContainer(run: EngineRun, definition: TFlowDefinition, step: Step, cursor: number, context: Record<string, unknown>): Promise<void> {
    ...
    if (step.type === "branch") {
      ...
      const inline = await this.runInline(run, definition, selected as Step[], context);
  ...
  private async executeLoop(run: EngineRun, definition: TFlowDefinition, step: any, cursor: number, context: Record<string, unknown>): Promise<void> {
    const props = resolveProps(step.props ?? {}, context); const items = props.items;
    if (!Array.isArray(items)) throw new EngineError("validation", "PROP_TYPE_MISMATCH");
    const concurrency = Math.max(1, Math.min(20, Number(props.concurrency ?? 1)));
    const outputs: unknown[] = new Array(items.length);
    const basePath = Array.isArray(context.__loopPath) ? context.__loopPath as string[] : [];
    if (concurrency === 1) {
      for (let i = 0; i < items.length; i++) outputs[i] = await this.runInline(run, definition, step.steps, { ...context, __loopPath: [...basePath, `${step.id}:${i}`], loop: { item: items[i], index: i, total: items.length } });
    } else {
      let next = 0;
      const workers = Array.from({ length: Math.min(concurrency, items.length) }, async () => {
        while (next < items.length) {
          const i = next++;
          outputs[i] = await this.runInline(run, definition, step.steps, { ...context, __loopPath: [...basePath, `${step.id}:${i}`], loop: { item: items[i], index: i, total: items.length } });
        }
      });
  ...
  private async executeStepRecursive(run: EngineRun, definition: TFlowDefinition, step: Step, context: Record<string, unknown>, depth: number): Promise<"ok" | "halt"> {
    ...
    if (step.type === "router") {
      ...
      context[step.id] = { branchId: selected?.id ?? null, ...output }; return "ok";
    }
    if (step.type === "loop") {
      const props = resolveProps((step as any).props ?? {}, context); const items = Array.isArray(props.items) ? props.items : [];
      const outputs: unknown[] = []; const basePath = Array.isArray(context.__loopPath) ? context.__loopPath as string[] : [];
      for (let i = 0; i < items.length; i++) outputs.push(await this.runInline(run, definition, (step as any).steps, { ...context, __loopPath: [...basePath, `${step.id}:${i}`], loop: { item: items[i], index: i, total: items.length } }, depth + 1));
      context[step.id] = { items: outputs, count: items.length }; return "ok";
    }
    ...
    if (step.type === "sub_flow") {
      ...
      for (const childStep of childDef.steps) await this.executeStepRecursive(run, childDef, childStep, childContext, depth + 1);
      context[step.id] = { status: "succeeded", flowId: childFlowId, steps: childContext }; return "ok";
    }
    if (step.type === "aggregator") {
      const props = resolveProps((step.props ?? {}) as Record<string, unknown>, context) as { sources?: string[]; mode?: string; separator?: string };
      context[step.id] = aggregateOutput(props.sources ?? [], props.mode, props.separator, context); return "ok";
    }
    const props = resolveProps((step as Record<string, unknown>).props as Record<string, unknown> ?? {}, context);
    const result = await this.executeLeaf(run, step, props, context);
    ...
    if (result.kind === "error" && result.error.errorClass !== "auth" && (policy === "continue" || policy === "fallback")) {
      context[step.id] = policy === "fallback" && fallbackValue !== undefined ? (resolveValue(fallbackValue, context) as Record<string, unknown>) : { error: { message: result.error.message, code: result.error.code } };
      return "ok";
    }
  ...
  private async executeLeaf(run: EngineRun, step: Step, props: Record<string, unknown>, context: Record<string, unknown>): Promise<Outcome> {
    ...
    if (!handler) return { kind: "error", error: new EngineError("fatal", "NO_HANDLER", `No handler registered for step type ${step.type}`) };
    const retry = retryPolicy(step); let last: Outcome = { kind: "error", error: new EngineError("fatal", "NO_ATTEMPT") };
    const loopPath = Array.isArray(context.__loopPath) ? context.__loopPath.join("/") : "root";
    const key = `${run.id}:${loopPath}:${step.id}:effect`;
    const handlerContext = Object.fromEntries(Object.entries(context).filter(([k]) => k !== "__loopPath"));
    for (let attempt = 1; attempt <= retry.maxAttempts; attempt += 1) {
      const started = Date.now();
      const completed = await this.db.runSteps.completedByEffectKey(run.id, step.id, key);
      if (completed) return { kind: "ok", output: completed.outputJson ?? {} };
      try {
        last = await Promise.race([
          handler.execute({ run, step, props, context: Object.freeze({ ...handlerContext }), attempt, idempotencyKey: key }),
          new Promise<Outcome>((_, reject) => setTimeout(() => reject(new EngineError("transient", "TIMEOUT")), stepTimeout(step))),
        ]);
      } catch (error) { last = { kind: "error", error: EngineError.from(error) }; }
      await this.record(run, step, props, last, Date.now() - started, attempt, key);
  ...
  private async scheduleResume(run: EngineRun, _cursor: number, resumeAt: string): Promise<void> {
    const delayMs = Math.max(0, new Date(resumeAt).getTime() - Date.now());
    if (!this.queues?.flowStep?.add) return;
    await this.queues.flowStep.add("resume", { runId: run.id, orgId: run.orgId, kind: "resume" }, { jobId: `resume:${run.id}:${resumeAt}`, delay: delayMs, removeOnComplete: 1000 });
  }

  private async pause(run: EngineRun, cursor: number, context: Record<string, unknown>, outcome: { reason: string; resumeAt?: string }, nextCursor = cursor + 1): Promise<void> {
    await this.db.flowRuns.pause(run.id, { expectedCursor: cursor, expectedEpoch: run.transitionEpoch ?? 1, contextJson: context, reason: outcome.reason, resumeAt: outcome.resumeAt ?? null, nextCursor });
  }
  ...
  private async applyErrorPolicy(run: EngineRun, definition: TFlowDefinition, step: Step, cursor: number, context: Record<string, unknown>, error: EngineError): Promise<void> {
    ...
    if (error.errorClass !== "auth") {
      if (policy === "continue" || policy === "fallback") {
        ...
        return this.advance(run, cursor, { [step.id]: fallback as Record<string, unknown> }, definition.steps.length);
      }
      if (policy !== "fail" && policy !== "stop") {
        const targetIdx = definition.steps.findIndex((s) => s.id === policy);
        if (targetIdx > cursor) {
          const state = await this.db.flowRuns.checkpoint(run.id, { expectedCursor: cursor, expectedEpoch: run.transitionEpoch, appendContext: { [step.id]: errorOutput }, nextCursor: targetIdx, status: "queued" });
          await this.queues.flowStep.add("transition", { runId: run.id, orgId: run.orgId, cursor: state.cursor, epoch: state.transitionEpoch }, { jobId: `step:${run.id}:${state.cursor}:${state.transitionEpoch}` });
          return;
        }
      }
  ...
  private async record(run: EngineRun, step: Step, input: unknown, outcome: Outcome, durationMs: number, attempt: number, effectKey: string): Promise<void> {
    const output = outcome.kind === "ok" ? outcome.output : undefined;
    await this.db.runSteps.insert({
      runId: run.id,
      ...
      errorClass: outcome.kind === "error" ? outcome.error.errorClass : undefined,
      errorCode: outcome.kind === "error" ? outcome.error.code : undefined,
      attempt,
      durationMs,
    });
  }

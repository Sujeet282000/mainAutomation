...
export interface StepHandler {
  execute(ctx: {
    run: { id: string; orgId: string; mode?: string };
    step: Step;
...
function stepTimeout(step: Step): number {
  ...
  if (props?.timeoutMs) return Number(props.timeoutMs);
...
export class Executor {
  ...
  async transition(runId: string, cursor: number, epoch: number): Promise<void> {
    ...
    // Filter: evaluate condition, skip if false
    if (step.type === "filter") {
      if (!evaluateFlowCondition(step.condition as any, context)) {
    ...
    // Loop
    if (step.type === "loop") {
      return this.executeLoop(run, definition, step as any, cursor, context);
    }

    // Note: no-op
    if (step.type === "note") {
      return this.advance(run, cursor, { [step.id]: { noted: true } });
    }

    // Delay: dehydrate
    if (step.type === "delay") {
      return this.pause(run, cursor, context, {
        kind: "pause",
        reason: "delay",
        resumeAt: (step as any).props?.untilIso,
      });
    }

    // Approval: create todo, dehydrate
    if (step.type === "approval") {
      const props = (step as any).props || {};
      await this.db.todos.create(
        run.orgId, run.id, run.createdAt, step.id,
        props.title || "Approval needed",
    ...
    // Leaf steps: execute via handler
    const props = resolveProps(
      (step as Record<string, unknown>).props as Record<string, unknown> ?? {},
      context,
    );
    const result = await this.executeLeaf(run, step, props, context);

    if (result.kind === "ok") {
      return this.advance(run, cursor, { [step.id]: result.output });
    }
    if (result.kind === "stop") {
      return this.finish(run.id, "filtered", context);
    }
  ...
  private async executeLeaf(
    ...
    for (let attempt = 1; attempt <= retry; attempt += 1) {
      ...
      // Idempotency: check for completed effect
      ...
      try {
        last = await Promise.race([
          ...
          new Promise<Outcome>((_, reject) =>
            setTimeout(() => reject(new EngineError("transient", "TIMEOUT")), stepTimeout(step)),
          ),
        ]);
      } catch (error) {
  ...
  private async executeContainer(
    run: { id: string; orgId: string; contextJson?: unknown },
    definition: TFlowDefinition,
    step: Step,
    cursor: number,
    context: Record<string, unknown>,
  ): Promise<void> {
    if (step.type === "branch") {
      const selected = evaluateFlowCondition(step.condition as any, context)
        ? step.onTrue
        : step.onFalse;
      const output = await this.runInline(run, definition, selected as Step[], context);
      return this.advance(run, cursor, { [step.id]: output });
    }

    if (step.type === "router") {
      const branches = (step as any).branches;
      const selected = branches.find((b: any) => b.condition && evaluateFlowCondition(b.condition, context))
        ?? branches.find((b: any) => b.default);
      const output = selected
        ? await this.runInline(run, definition, selected.steps, context)
        : {};
      return this.advance(run, cursor, {
        [step.id]: { branchId: selected?.id ?? null, ...output },
      });
    }
  ...
  private async executeLoop(
    run: { id: string; orgId: string; transitionEpoch?: number },
    definition: TFlowDefinition,
    step: any,
    cursor: number,
    context: Record<string, unknown>,
    ...
    const concurrency = Math.min(20, Math.max(1, Number(props.concurrency ?? 1)));
    const outputs: unknown[] = new Array(items.length);

    // Sequential execution for now; parallel with p-limit in production
    for (let i = 0; i < items.length; i++) {
      const child = { ...context, loop: { item: items[i], index: i, total: items.length } };
      outputs[i] = await this.runInline(run, definition, step.steps, child);
    }

    await this.advance(run, cursor, {
      [step.id]: { items: outputs, count: items.length },
    });
  }
  ...
  private async runInline(
    ...
    for (const child of steps) {
      if (child.type === "filter" && !evaluateFlowCondition(child.condition as any, { ...context, ...appended })) {
        break;
      }
  ...
  private async advance(
    ...
    const state = await this.db.flowRuns.checkpoint(run.id, {
      expectedCursor: cursor,
      expectedEpoch: run.transitionEpoch,
      appendContext: append,
      nextCursor: cursor + 1,
      status: "queued",
    });

    await this.queues.flowStep.add("transition", {
      runId: run.id,
      orgId: run.orgId,
      cursor: state.cursor,
      epoch: state.transitionEpoch,
    }, {
      jobId: `step:${run.id}:${state.cursor}:${state.transitionEpoch}`,
    });
  }

  private async pause(
    run: { id: string; transitionEpoch?: number },
    cursor: number,
    context: Record<string, unknown>,
    outcome: { kind?: string; reason: string; resumeAt?: string },
  ): Promise<void> {
    await this.db.flowRuns.pause(run.id, {
      expectedCursor: cursor,
      expectedEpoch: run.transitionEpoch ?? 1,
      contextJson: context,
  ...
  private async applyErrorPolicy(
    run: { id: string },
    definition: TFlowDefinition,
    step: Step,
    cursor: number,
    ...
    if (policy === "continue") {
      return this.advance(run as any, cursor, { [step.id]: { error: { message: error.message, code: error.code } } });
    }

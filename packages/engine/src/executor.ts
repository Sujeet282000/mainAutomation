// ============================================================================
// Orchestra Part 6 — The Executor
// Source of truth: Part 6 § "The Executor"
// The executor leases a transition epoch, loads the run's pinned version,
// and walks from the durable cursor.
// ============================================================================
...
type Outcome =
  ...
  | { kind: "stop" }
  | { kind: "pause"; reason: string; resumeAt?: string };

export interface StepHandler {
  execute(ctx: {
    run: { id: string; orgId: string; projectId?: string; mode?: string };
    step: Step;
...
function resolveProps(props: Record<string, unknown>, context: Record<string, unknown>): Record<string, unknown> {
  const result: Record<string, unknown> = {};
  for (const [key, value] of Object.entries(props)) result[key] = resolveValue(value, context);
  return result;
}

function stepTimeout(step: Step): number {
  const props = (step as Record<string, unknown>).props as Record<string, unknown> | undefined;
  return props?.timeoutMs ? Number(props.timeoutMs) : 30_000;
}

function backoff(attempt: number): number {
  return Math.min(1000 * Math.pow(2, attempt - 1), 60_000) + Math.random() * 1000;
}
...
export class Executor {
  constructor(
    ...
    private readonly handlers: Map<string, StepHandler>,
  ) {}
  ...
  async transition(runId: string, cursor: number, epoch: number): Promise<void> {
    ...
    if (!version) throw new EngineError("fatal", "FLOW_VERSION_NOT_FOUND");
    const definition = version.definition as TFlowDefinition;
    const steps = definition.steps;
    if (cursor >= steps.length) return this.finish(run.id, "succeeded", run.context);

    const step = steps[cursor];
    const context = run.context as Record<string, unknown>;

    if (step.type === "filter") {
      if (!evaluateFlowCondition(step.condition as any, context)) return this.finish(run.id, "filtered", context);
      return this.advance(run, cursor, { [step.id]: { passed: true } });
    }
  ...
  private async executeLeaf(
    run: { id: string; orgId: string; projectId?: string },
    step: Step,
    props: Record<string, unknown>,
    context: Record<string, unknown>,
  ): Promise<Outcome> {
    const handler = this.handlers.get(step.type);
    if (!handler) return { kind: "error", error: new EngineError("fatal", "NO_HANDLER") };
    const retry = (step as any).retry?.maxAttempts ?? 3;
    let last: Outcome = { kind: "error", error: new EngineError("fatal", "NO_ATTEMPT") };
    ...
    for (let attempt = 1; attempt <= retry; attempt += 1) {
      ...
      try {
        last = await Promise.race([
          handler.execute({ run, step, props, context: Object.freeze({ ...context }), attempt, idempotencyKey: key }),
  ...
  private async executeContainer(run: any, definition: TFlowDefinition, step: Step, cursor: number, context: Record<string, unknown>): Promise<void> {
    if (step.type === "branch") {
      const selected = evaluateFlowCondition(step.condition as any, context) ? step.onTrue : step.onFalse;
      return this.advance(run, cursor, { [step.id]: await this.runInline(run, definition, selected as Step[], context) });
    }
    if (step.type === "router") {
      const branches = (step as any).branches;
      const selected = branches.find((b: any) => b.condition && evaluateFlowCondition(b.condition, context)) ?? branches.find((b: any) => b.default);
      const output = selected ? await this.runInline(run, definition, selected.steps, context) : {};
      return this.advance(run, cursor, { [step.id]: { branchId: selected?.id ?? null, ...output } });
    }
    return this.advance(run, cursor, {});
  }

  private async executeLoop(run: any, definition: TFlowDefinition, step: any, cursor: number, context: Record<string, unknown>): Promise<void> {
    ...
    return this.advance(run, cursor, { [step.id]: { items: outputs, count: items.length } });
  }

  private async runInline(run: any, definition: TFlowDefinition, steps: Step[], context: Record<string, unknown>): Promise<Record<string, unknown>> {
    ...
    for (const child of steps) {
      ...
      if (child.type !== "filter") {
        const props = resolveProps((child as any).props ?? {}, { ...context, ...appended });
        const result = await this.executeLeaf(run, child, props, { ...context, ...appended });
        if (result.kind !== "ok") throw new EngineError("fatal", `INLINE_${result.kind.toUpperCase()}`);
  ...
  private async advance(run: any, cursor: number, append: Record<string, unknown>): Promise<void> {
  ...
  private async pause(run: any, cursor: number, context: Record<string, unknown>, outcome: { kind?: string; reason: string; resumeAt?: string }): Promise<void> {
    await this.db.flowRuns.pause(run.id, { expectedCursor: cursor, expectedEpoch: run.transitionEpoch ?? 1, contextJson: context, reason: outcome.reason, resumeAt: outcome.resumeAt ?? null });
  }

  private async applyErrorPolicy(run: any, definition: TFlowDefinition, step: Step, cursor: number, context: Record<string, unknown>, error: EngineError): Promise<void> {
    const policy = (step as any).onError ?? "fail";
    if (policy === "continue") return this.advance(run, cursor, { [step.id]: { error: { message: error.message, code: error.code } } });
    return this.finish(run.id, "failed", { ...context, [step.id]: { error: { message: error.message, code: error.code } } });
  }

  private async record(run: any, step: Step, input: unknown, outcome: Outcome, durationMs: number, attempt: number, effectKey: string): Promise<void> {
    const output = outcome.kind === "ok" ? outcome.output : null;
    await this.db.runSteps.insert({ runId: run.id, runCreatedAt: run.createdAt, orgId: run.orgId, stepId: step.id, stepType: step.type, effectKey, status: outcome.kind === "ok" ? "succeeded" : "failed", inputJson: input as Record<string, unknown>, outputJson: output ?? undefined, errorClass: outcome.kind === "error" ? outcome.error.errorClass : undefined, errorCode: outcome.kind === "error" ? outcome.error.code : undefined, attempt, durationMs });
  }

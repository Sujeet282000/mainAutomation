...
function interpolate(template, context) {
    return template.replace(TOKEN_PATTERN, (_match, expr) => {
    ...
    switch (name) {
        ...
        case "replace":
            return String(value ?? "")
                ...
                .join(args[1] ?? "");
        case "slice":
            return String(value ?? "").slice(Number(args[0] ?? 0), numberOrUndefined(args[1]));
        case "first":
...
function evaluateCondition(condition, context) {
    if (condition.op === "and") {
        return (condition.operands ?? []).every((item) => evaluateCondition(item, context));
    }
    if (condition.op === "or") {
        return (condition.operands ?? []).some((item) => evaluateCondition(item, context));
    }
    if (condition.op === "not") {
    ...
    switch (condition.op) {
        ...
        case "matches": {
            ...
            if (pattern.length > MAX_REGEX_PATTERN_LENGTH) {
                throw new ExpressionError("FILTER_ERROR", `Regex pattern exceeds max length: ${MAX_REGEX_PATTERN_LENGTH}`);
            }
